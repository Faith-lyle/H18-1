...
def write_log(file_name, text, results):
    ...
    if os.path.isfile(file_path1):
        ...
        with open(file_path1, 'w') as f:
            f.write('Test Time,SN,Link Check,Read SN,Open Tunnel,MIC3 Test,Optical Sensor Test,PCM Test,Gcal Test,'
                    'Close Tunnel,Result\n')
    # print file_path
...
class Application(Frame):
    ...
    def test_widget(self):
        ...
        self.sheet1.insert('', index=2, value=('3', 'Open Tunnel', 'NA', 'NA',))
        self.sheet1.insert('', index=3, value=('4', 'MIC3 Test', 'NA', 'NA',))
        self.sheet1.insert('', index=4, value=('5', 'Optical Sensor Test', spec_lo, spec_up,))
        self.sheet1.insert('', index=5, value=('6', 'PCM Test', 'NA', 'NA',))
        self.sheet1.insert('', index=6, value=('7', 'Gcal Test', '8575', '8900',))
    ...
    def showlog_widght(self, master):
        ...
        L4 = Label(master, text='Author:电子开发应用二课，Lyle.Hou      version:3.1  ', bg='#6A5ACD', fg='white',
                   anchor='w', font=('黑体', 11))
    ...
    def port_choose(self, master):
        ...
        if self.lb11.get(0) == '':
            ...
            if self.term == None:
                self.term = serial.Serial(port=port1, baudrate=921600, timeout=0.5)
                self.port_name.set(value='Serial Port is: {}'.format(self.term.port))
            else:
    ...
    def begin1(self):
        ...
        if self.term != None:
            try:
                ...
                time.sleep(0.05)
                self.term.write('[ttim-015]'.encode('utf-8'))
                time.sleep(0.05)
                ...
                self.term.flushInput()
                self.term.flushOutput()
                data_loc = re.compile(r'] ft uvp(.*?)\n', re.S)
                print(value8)
                value = re.search(data_loc, value7)
                print(value)
                text += value8
                if value is None:
                    print('ok')
                    ...
                    return
                while True:
                    # print(self.term.in_waiting)
                    if self.term.in_waiting:
                        print(self.term.read(self.term.in_waiting))
                        break
                self.term.write('[sr]\n'.encode('utf-8'))
                # self.term.write('[sr]\n'.encode('utf-8'))
                # self.term.write('[sr]\n'.encode('utf-8'))
                ...
                self.term.write('audio stop\n'.encode('utf-8'))
                time.sleep(0.1)
                value9 = self.term.read(5000).decode('utf-8')
                text += value9
                self.term.flushInput()
                self.term.flushOutput()
                print(value9)
                side8 = value9.find('audio hp_cal')
                try:
                    Gcal = int(value9[side8 + 19:side8 + 23])
                except Exception:
                    self.paly('fail')
